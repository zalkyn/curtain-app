...
export const action = async ({ request, params }) => {
    ...
    if (role === "createSwatch") {
        ...
        const newSwatch = await prisma.borderSwatch.create({
            data: {
                ...parsedData,
                shop: session?.shop
            }
    ...
    if (role === "editSwatch") {
        ...
        const updatedSwatch = await prisma.borderSwatch.update({
    ...
    if (role === "deleteSwatch") {
        ...
        const swatch = await prisma.borderSwatch.delete({
...
export default function Border() {
    ...
    const [border, setBorder] = useState(customizer?.border[0] || null)
    const [reserveBorder, setReserveBorder] = useState(customizer?.border[0] || null)
    const [swatches, setSwatches] = useState(customizer?.border[0]?.swatches || [])
    const [loading, setLoading] = useState({
        editSwatch: false,
        createSwatch: false,
        deleteSwatch: false,
        saveSwatch: false,
        updateBorderInfo: false
    })
    ...
    useEffect(() => {
        if (loaderData && loaderData?.customizer) {
            ...
            setBorder(loaderData?.customizer?.border[0] || null)
            setSwatches(loaderData?.customizer?.border[0]?.swatches || [])
            setReserveBorder(loaderData?.customizer?.border[0] || null);
        }
    }, [loaderData])
    ...
    useEffect(() => {
        ...
        if (actionData && actionData?.role === "updateBorderInfo") {
            if (actionData.border) {
                setBorder(actionData.border);
                setLoading({ ...loading, updateBorderInfo: false });
                shopify.toast.show("Border info updated successfully")
            }
        }
    }, [actionData])
    ...
    const handleCreateSwatch = async () => {
        ...
        let data = {
            ...
            activeStatus: swatchInput.activeStatus
        };
    ...
    const handleEditSwatch = async () => {
        ...
        let data = {
            ...
            activeStatus: selectedSwatch.activeStatus
        };
        const formData = new FormData();
        formData.append("data", JSON.stringify(data));
    ...
    const handleUpdateBorderInfo = async () => {
        setLoading({ ...loading, updateBorderInfo: true });
        ...
        <Layout>
                            ...
                            <InlineStack align="space-between" blockAlign="center" gap={400}>
                                ...
                                <Box>
                                    <Box paddingBlockEnd={300}>
                                        ...
                                        <Text variant="headingMd">Leading Edge Price: <Badge>${swatch.price?.leadingEdge.toFixed(2)}</Badge></Text>
                                        <Box paddingBlockEnd={200} />
                                        <Text variant="headingMd">Bottom Price: <Badge>${swatch.price?.bottom.toFixed(2)}</Badge></Text>
                                        <Box paddingBlockEnd={200} />
                                        <Text variant="headingMd">Leading Edge & Bottom Price: <Badge>${swatch.price?.leadingEdgeBottom.toFixed(2)}</Badge></Text>
                                    </Box>
                                    <ButtonGroup>
                                        <Button
                                            icon={EditIcon}
            ...
            <Layout.Section variant="fullWidth">
                ...
                <Card>
                    ...
                    <Checkbox
                        label="Active Status"
                        checked={border?.activeStatus || false}
                        onChange={(value) => setBorder({ ...border, activeStatus: value })}
                    />
                    ...
                    <Suspense fallback={<Text>Loading...</Text>}>
                        <TextEditor
                            showImage={true}
                            content={border?.info || ""}
                            setContent={(value) => setBorder({ ...border, info: value })}
                        />
                    </Suspense>
                </Card>
            </Layout.Section>
            ...
            <Layout.Section variant="fullWidth">
                ...
                >
                    <Modal.Section>
                        <FormLayout>
                            ...
                            {/* leadingEdge price  */}
                            <TextField
                                label="Leading Edge Price"
                                type="number"
                                value={swatchInput.price.leadingEdge}
                                onChange={(value) => setSwatchInput({
                                    ...swatchInput,
                                    price: {
                                        ...swatchInput.price,
                                        leadingEdge: parseFloat(value)
                                    }
                                })}
                                min={0}
                            />
                            ...
                            <TextField
                                label="Bottom Price"
                                type="number"
                                value={swatchInput.price.bottom}
                                onChange={(value) => setSwatchInput({
                                    ...swatchInput,
                                    price: {
                                        ...swatchInput.price,
                                ...
                                min={0}
                            />
                            {/* leadingEdgeBottom price  */}
                            <TextField
                                label="Leading Edge & Bottom Price"
                                type="number"
                                value={swatchInput.price.leadingEdgeBottom}
                                onChange={(value) => setSwatchInput({
                                    ...swatchInput,
                                    price: {
                                        ...swatchInput.price,
                                        leadingEdgeBottom: parseFloat(value)
                                    }
                                })}
                                min={0}
                            />
                    ...
                    <Modal.Section>
                        <FormLayout>
                            ...
                            <TextField
                                label="Leading Edge Price"
                                type="number"
                                value={selectedSwatch?.price?.leadingEdge}
                                onChange={(value) => setSelectedSwatch({
                                    ...selectedSwatch,
                                    price: {
                            ...
                            <TextField
                                label="Bottom Price"
                                type="number"
                                value={selectedSwatch?.price?.bottom}
                                onChange={(value) => setSelectedSwatch({
                                    ...selectedSwatch,
                                    price: {
                                        ...selectedSwatch.price,
                            ...
                            <TextField
                                label="Leading Edge & Bottom Price"
                                type="number"
                                value={selectedSwatch?.price?.leadingEdgeBottom}
                                onChange={(value) => setSelectedSwatch({
                                    ...selectedSwatch,
                                    price: {
                                        ...selectedSwatch.price,
                ...
                >
                    <Modal.Section>
                        <Text variant="bodyMd">Are you sure you want to delete the border <Badge><strong>{selectedSwatch?.title}</strong></Badge>? This action cannot be undone.</Text>
                    </Modal.Section>
                </Modal>
            </Layout.Section>
        </Layout >
