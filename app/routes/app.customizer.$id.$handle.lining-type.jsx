...
export default function CustomizerLiningType() {
    ...
    const [itmes, setItems] = useState(loaderData?.customizer?.liningType[0]?.items || []);
    ...
    // actoindata effect
    useEffect(() => {
        if (actionData && actionData?.role === "create") {
            setItems([...itmes, actionData?.liningItems]);
        ...
        } else if (actionData && actionData?.role === "delete-item") {
            const updatedItems = itmes.filter(item => item.id !== actionData?.deletedItem?.id);
            setItems(updatedItems);
            setModal({ ...modal, delete: false });
            setLoading({ ...loading, deleteBtn: false });
            shopify.toast.show("Lining Type deleted successfully")
        } else if (actionData && actionData?.role === "edit-item") {
            const updatedItems = itmes.map(item => {
    ...
    >
        <Layout>
            ...
            {itmes?.map((item, index) => {
                return <Layout.Section key={index} variant="oneHalf">
                    <Card>
                        <InlineStack align="start" gap={400}>
                            ...
                            <Box>
                                ...
                                <InlineStack gap={200} align="center">
                                    ...
                                    <Button icon={DeleteIcon} destructive onClick={() => {
                                        setSelectedItem(item);
            ...
            <Layout.Section>
                                ...
                                <InlineStack align="space-between" blockAlign="end">
                                    <Box>
                                        ...
                                        <InlineStack align="center">
                                            <DropZone onDrop={(_dropFiles, acceptedFiles) => handleDropZoneDrop(_dropFiles, acceptedFiles, 'primary')} outline={false} variableHeight={true} accept={validImageTypes} disabled={loading?.createBtn}>
                                                {newItem?.image ?
                                                    <Box>
                ...
                <Modal
                    ...
                    secondaryActions={[
                        ...
                        {
                            content: "Reset",
                            // disabled reset btn if value is same as selected item
                        ...
                        <FormLayout>
                            <Box>
                                <InlineStack align="space-between" blockAlign="end">
                                    <Box>
                                        <InlineStack align="center">
                                            <DropZone onDrop={(_dropFiles, acceptedFiles) => handleDropZoneDrop(_dropFiles, acceptedFiles, 'primary')} outline={false} variableHeight={true} accept={validImageTypes} disabled={loading?.editBtn}>
                                                {newItem?.image ?
                                                    <Box>
                                                        <img style={{ width: '120px', height: 'auto' }} src={
                                                            validImageTypes.includes(newItem?.image?.type)
