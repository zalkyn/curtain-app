...
import { Box, ButtonGroup, Button, Card, InlineStack, Badge, Text, Modal, FormLayout, TextField, Divider, Collapsible } from "@shopify/polaris";
import { useEffect, useState } from "react";
import { ChevronDownIcon, ChevronUpIcon, DeleteIcon, ViewIcon } from '@shopify/polaris-icons';

export default function SizeWidthGroup() {
    ...
    const [reserveGroup, setReserveGroup] = useState(loaderData?.panelSize?.widthGroup)
    const [group, setGroup] = useState(loaderData?.panelSize?.widthGroup)
    ...
    useEffect(() => {
        if (loaderData) {
            ...
            setGroup(loaderData?.panelSize?.widthGroup)
            setReserveGroup(loaderData?.panelSize?.widthGroup)
        }
    }, [loaderData])
    ...
    useEffect(() => {
        if (newGroup.minWidth < newGroup.maxWidth) {
            const newSizes = [];
            for (let i = newGroup.minWidth; i <= newGroup.maxWidth; i++) {
                const index = (i - newGroup.minWidth) + 0; // Calculate index starting from 0
                newSizes.push({
                    id: new Date().getTime() + i, // Ensure unique IDs
                    width: i, // Set width to i
                    customPrice: 0,
    ...
    const handleSelectedGroupInputChange = (_group, role, field, value, id = null) => {
        setSelectedGroup((prev) => {
            ...
            if (role === "group") {
                ...
                if (_prev.minWidth < _prev.maxWidth) {
                    for (let i = _prev.minWidth; i <= _prev.maxWidth; i++) {
                        const index = (i - _prev.minWidth); // Calculate index starting from 0

                        // Check if the size already exists in the group
                        const existingSizeIndex = _prev.group.findIndex(size => size.width === i);
                    ...
                    // Merge existing group with new sizes, then sort by width
                    _prev.group = [..._prev.group, ...newSizes].sort((a, b) => a.width - b.width);
                } else {
                    _prev.group = [];
                }

                // Handle removing sizes if the maxWidth has decreased (out of the new range)
                if (_prev.maxWidth < prev.maxWidth) {
                    // Remove any sizes where the width is greater than the new maxWidth
                    _prev.group = _prev.group.filter(size => size.width <= _prev.maxWidth);
                }

                // Handle removing sizes if the minWidth has increased (out of the new range)
                if (_prev.minWidth > prev.minWidth) {
                    // Remove any sizes where the width is less than the new minWidth
                    _prev.group = _prev.group.filter(size => size.width >= _prev.minWidth);
                }
            } else {
    ...
    return <Box>
        <Card>
            <InlineStack align="space-between" blockAlign="center">
                <Text variant="headingMd">Width Group [{group?.length || 0}]</Text>
                <ButtonGroup>
                    ...
                    return <Box key={`group--${gi}`} paddingBlockEnd={200}>
                        <Card>
                            <InlineStack align="space-between" blockAlign="center">
                                <Box>
                                    <InlineStack gap={200}>
                                        <Text variant="headingMd">{g.minWidth} inch - {g.maxWidth} inch</Text>
                ...
                <FormLayout>
                    <FormLayout.Group condensed>
                        <TextField
                            type="number"
                            label="Min Width"
                            value={newGroup.minWidth ? newGroup.minWidth : ""}
                            onChange={(value) => handleChangeNewInput("minWidth", parseFloat(value))}
                            min={0}
                            prefix="Inch"
                        />
                        <TextField
                            type="number"
                            label="Max Width"
                            value={newGroup.maxWidth ? newGroup.maxWidth : ""}
                            onChange={(value) => handleChangeNewInput("maxWidth", parseFloat(value))}
                            min={0}
                            prefix="Inch"
                        />
                    </FormLayout.Group>
                            ...
                            <FormLayout>
                                <FormLayout.Group condensed>
                                    <TextField
                                        ...
                                        value={size?.width}
                                        disabled
                                        onChange={(value) => handleSizesInputChange(size?.id, "width", value)}
                                    />
                                    <TextField
                                        type="number"
                                        label="Price"
        ...
        {/* view group modal  */}
        <Modal
            title={`Width Group [${selectedGroup?.minWidth} - ${selectedGroup?.maxWidth}] inch`}
            open={viewModal}
                        ...
                        <FormLayout>
                            <FormLayout.Group condensed>
                                <TextField
                                    ...
                                    prefix="Min Width: "
                                    suffix="inch"
                                    value={selectedGroup.minWidth}
                                    onChange={(value) => handleSelectedGroupInputChange(selectedGroup, "group", "minWidth", parseFloat(value))}
                                    disabled={loadingUpdateBtn}
                                />
                                <TextField
                                    type="number"
                                    ...
                                    prefix="Max Width: "
                                    suffix="inch"
                                    value={selectedGroup.maxWidth}
                                    onChange={(value) => handleSelectedGroupInputChange(selectedGroup, "group", "maxWidth", parseFloat(value))}
                                    disabled={loadingUpdateBtn}
                                />
                            </FormLayout.Group>
                        </FormLayout>
